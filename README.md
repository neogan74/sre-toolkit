...
- Chaos mock server with latency, 5xx, timeout, and connection-failure injection
...
# Run chaos mock server with dropped connections
chaos-load mock --port 8080 --error-rate 20 --connection-failure-rate 10 --timeout-rate 5 --timeout-duration 12s --latency 150ms
```
